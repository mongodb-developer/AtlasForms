...
async function generateDefaultSchemaInfo (namespace) {
  ...
  try {
    ...
    for (const exampleDoc of exampleDocs) {
...
function addDocumentToTemplate (doc, templateDoc) {
  ...
  for (const key of Object.keys(doc)) {
    if (typeof doc[key] === 'object') {
      const bsonType = utilityFunctions.getBsonType(doc[key])
...
function getSystemDocTypeSchemaInfo (namespace) {
  ...
  if (namespace === '__atlasforms.users') {
    return {
      _id: 'string:24',
      type: 'string:6',
      data: { email: 'string:23' },
      permissions: [{ item: 'string:0', permissions: 'string:0' }],
      isSuperUser: 'boolean',
      custom_data: {},
      identities: [{ id: 'string:24', provider_type: 'string:14' }],
      createdate: 'date'
    }
  }
}
